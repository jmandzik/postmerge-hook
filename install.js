...
var pcPath = path.join(gitPath, 'hooks', 'post-merge');
...
if (existsSync(gitPath)) {
    ...
    if (stats.isDirectory()) {
        ...
        console.log('Found .git directory, adding post-merge hook');
        var pcHook = fs.readFileSync(path.join(filePath, 'post-merge'));
        fs.writeFileSync(pcPath, pcHook);
        fs.chmodSync(pcPath, '755');
    }
} else if (existsSync(pcModulePath)){
    console.log('Found submodule .git directory, adding post-merge hook');
    var pcHook = fs.readFileSync(path.join(filePath, 'post-merge'));
    var pcModuleFullPath = path.join(pcModulePath, 'post-merge');
    fs.writeFileSync(pcModuleFullPath, pcHook);
    fs.chmodSync(pcModuleFullPath, '755');
} else {
    console.error('This project doesn\'t appear to be a git repository. JSHint configuration will be created anyway. To enable the post-merge hook, run `git init` and reinstall precommit-hook.');
}
